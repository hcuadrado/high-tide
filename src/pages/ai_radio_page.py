...
class HTAIRadioPage(Page):
    ...
    __gsignals__ = {
        ...
        "playlist-saved": (GObject.SignalFlags.RUN_FIRST, None, ()),
    }
    ...
    def _th_save_as_playlist(self) -> None:
        ...
        try:
            ...
            playlist.add([t.id for t in self._tracks])
        except Exception:
            logger.exception("Failed to save AI Radio playlist")
            GLib.idle_add(self._on_save_complete, 0, total)
            return
        GLib.idle_add(self._on_save_complete, total, total)
